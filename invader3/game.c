...
_S_BULLET_OBJECT gBulletObjects2[4];
...
int main()
{
	...
	map_load(&gBulletModel2,"bullet5.dat");
	//외계인
	...
	while(bLoop) {
		...
		if(acc_guid_delay_tick > 3) {
			for(int i=0;i<32;i++) {
				gBulletObjects[i].pfFire(&gBulletObjects[i],gAlienObjects[i].m_fXpos,
				gAlienObjects[i].m_fYpos,10,0,1,5);
				gBulletObjects[i].m_nFSM = 1;
			}
		...
		*/
		...
		for(int i=0;i<4;i++) {
			if(gBulletObjects2[i].m_nFSM != 0) {
				double target_posx2=gAlienObjects[i].m_fXpos;
				double target_posy2=gAlienObjects[i].m_fYpos;

				double bullet_posx2=gBulletObjects2[i].m_fXpos;
				double bullet_posy2=gBulletObjects2[i].m_fYpos;

				double vx2=bullet_posx2-target_posx2;
				double vy2=bullet_posy2-target_posy2;
